...
class NeRFRenderer(nn.Module):
    ...
    #     pose_GT = mn.get_all_camera_poses(opt).to(opt.device)
    #     # add synthetic pose perturbation to all training data
    #     pose = mn.get_all_camera_poses(opt).to(opt.device)
    #     # add learned pose correction to all training data
    #     import camera
    #     pose_refine = camera.lie.se3_to_SE3(self.model.self_graph.se3_refine.weight)
    ...
    def render(self, rays_o, rays_d, staged=False, max_ray_batch=4096, **kwargs):
        ...
        if staged and not self.cuda_ray: #X
            ...
            results = _run(rays_o, rays_d, **kwargs) #wights_sum,depth,image

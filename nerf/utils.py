...
class Trainer(object):
    def __init__(self, 
        ...
        #self.se3_refine = torch.nn.Embedding(len(loader_BARF), 6).to(opt_BARF.device)
        ...
        if optim_pose is None:
            ...
            self.optim_pose =  optimizer2([dict(params=self.self_graph.se3_refine.parameters(), lr=self.opt_BARF.optim.lr_pose)])
    ...
    def train_gui(self, train_loader, step=16):
        ...
        self.model.train()
        self.self_graph.train()
        self.self_graph.se3_refine.train()
        ...
        for _ in range(step):
            ...
            # #barf pose optimizer
            # import main_nerf as mn
            # mn.train_iteration_pose(self.opt_BARF, self.var_BARF, self.loader_BARF, self.global_step,self.optim_pose,self.self_graph,
            #                         self.sched_pose)
            # if self.global_step % 10 == 0:
            #     pose,pose_GT = mn.get_all_training_poses(self.opt_BARF,self.self_graph)
            #     _,sim3 = mn.prealign_cameras(self.opt_BARF,pose,pose_GT)
            ...
            for param in self.self_graph.se3_refine.parameters():
                # 파라미터에 대한 작업 수행
                gradient = param.grad
            ...
            import main_nerf as mn
            if self.global_step % 10 == 0:
                # self.train_iteration_pose(self.opt_BARF, self.var_BARF, self.loader_BARF, self.global_step,
                #                         self.optim_pose, self.self_graph,
                #                         self.sched_pose, loss2)
                self.train_iteration_pose()
            # opt123=self.opt_BARF
            # graph123=self.self_graph

            #     pose, pose_GT = mn.get_all_training_poses(opt=opt123, graph=graph123)
            #     _, sim3 = mn.prealign_cameras(self.opt_BARF, pose, pose_GT)
            #     loss_val = edict()  # validation할 때마다 포즈 업데이트
            #
            #     # newposes = []
